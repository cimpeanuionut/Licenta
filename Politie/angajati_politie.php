...
<?php
$dsn = 'mysql:host=localhost;dbname=politie';
	$username = 'root';
	$password = '';
	try{
	...
	if(isset($_POST['search']))
	{
		...
		if (empty($data[1]))
		{
			...
			$searchStmt = $con->prepare('SELECT * FROM angajati_politie WHERE name= :name');
	...
	if (empty($data[1]) || empty($data[2]) || empty($data[3])|| empty($data[4])|| empty($data[5]))
		{
			...
			$insertStmt = $con->prepare('INSERT INTO angajati_politie (name, prenume, email, adresa, telefon ) VALUES(:name,:prenume,:email , :adresa, :telefon)');
	...
	if ( empty($data[1]) || empty($data[2]) || empty($data[3])|| empty($data[4])|| empty($data[5]))
		{
			...
			$updateStmt = $con->prepare('UPDATE  angajati_politie SET name = :name, prenume = :prenume,email = :email,  adresa =:adresa, telefon =:telefon WHERE name = :name');
	...
	if (empty($data[1]))
		{
			...
			$deleteStmt = $con->prepare('DELETE FROM  angajati_politie WHERE name = :name');
...
$connect = mysqli_connect("localhost", "root", "", "politie");
	...
	foreach($objPHPExcel->getWorksheetIterator() as $worksheet)
 {
	 ...
	 for ($row=2; $row<=$highestRow; $row++)
	 {
		...
		$sql = "INSERT INTO angajati_politie(name, prenume,email, adresa,telefon) VALUES ( '" .$name."','" .$prenume."','" .$email."','" .$adresa."','".$telefon."')";
...
?>
	<form action="angajati_politie.php" method="POST">
	...
	<form action="angajati_politie.php" method="post" enctype="multipart/form-data">
